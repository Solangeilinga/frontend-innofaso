...
const RISQUE_CFG = {
  'ÉLEVÉ':  { badge:'bg-red-100 text-red-700',    dot:'bg-red-500',    bar:'bg-red-500',    reco:'⚠️ Planifier une maintenance' },
  'MODÉRÉ': { badge:'bg-orange-100 text-orange-700', dot:'bg-orange-400', bar:'bg-orange-400', reco:'👁 Surveiller de près' },
  'FAIBLE': { badge:'bg-green-100 text-green-700', dot:'bg-green-500',  bar:'bg-green-500',  reco:'✅ Aucune action requise' },
};

// Correspondance entre nom en base et nom dans le modèle IA
function trouverPrediction(predictions, nomEquipement) {
  if (!predictions?.length || !nomEquipement) return null;
  const nom = nomEquipement.toLowerCase().trim();

  // 1. Correspondance exacte
  let m = predictions.find(p => p.equipement?.toLowerCase().trim() === nom);
  if (m) return m;

  // 2. Nom DB contenu dans nom modèle
  m = predictions.find(p => p.equipement?.toLowerCase().includes(nom));
  if (m) return m;

  // 3. Nom modèle contenu dans nom DB
  m = predictions.find(p => nom.includes(p.equipement?.toLowerCase().trim()));
  if (m) return m;

  // 4. Les 2 premiers mots correspondent
  const deuxMots = nom.split(' ').slice(0, 2).join(' ');
  if (deuxMots.length > 3) {
    m = predictions.find(p => p.equipement?.toLowerCase().startsWith(deuxMots));
    if (m) return m;
  }
...
// Cache global des prédictions (évite de rappeler l'API pour chaque carte)
let cachePredsPromise = null;
let cachePreds = null;

function useIAPredictions() {
  const [preds, setPreds]     = useState(cachePreds);
  const [loading, setLoading] = useState(!cachePreds);

  useEffect(() => {
    if (cachePreds) { setPreds(cachePreds); setLoading(false); return; }
    if (!cachePredsPromise) {
      const now = new Date();
      cachePredsPromise = dashboardAPI.predictions({
        mois: now.getMonth() + 1,
        annee: now.getFullYear(),
      }).then(r => {
        cachePreds = r.data?.predictions || [];
        return cachePreds;
      }).catch(() => {
        cachePreds = [];
        return [];
...
// ── Widget IA par équipement ──────────────────────────────────────
function WidgetIA({ equipementNom }) {
  const { preds, loading } = useIAPredictions();
  const [open, setOpen] = useState(false);

  const pred = open ? trouverPrediction(preds, equipementNom) : null;
  const cfg  = pred ? (RISQUE_CFG[pred.risque] || RISQUE_CFG['FAIBLE']) : null;
        ...
        <div className="mt-2">
          ...
          {!loading && !pred && (
            <p className="text-xs text-muted-foreground italic py-1">
              Service IA non disponible ou équipement non couvert.
            </p>
          )}
          ...
          {!loading && pred && cfg && (
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between flex-wrap gap-1">
                ...
                <span className="text-xs text-muted-foreground">{cfg.reco}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <form onSubmit={sub} className="space-y-3">
          {[
            ...
            <div key={k}>
              ...
              <input value={f[k]} onChange={e => s(k, e.target.value)} placeholder={ph} className="input"/>
            </div>
          ))}
              ...
              {groupe.map(e => (
                <div key={e.id} className="card hover:shadow-lg transition-shadow">
                  ...
                  <div className="space-y-1 text-xs text-muted-foreground">
                    {e.type_equipement && <p>Type : <span className="text-foreground">{e.type_equipement}</span></p>}
                    {e.localisation    && <p className="flex items-center gap-1"><MapPin size={11}/>{e.localisation}</p>}
                    {e.ligne_production && <p>Ligne : <span className="text-foreground">{e.ligne_production}</span></p>}
                  </div>
